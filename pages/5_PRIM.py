...
st.title("🌲 Visualisation de l'algorithme de Prim")

st.markdown("""
Bienvenue dans l’espace dédié à **l’algorithme de Prim**.
...
- **Visualiser les arêtes ajoutées progressivement**
- **Voir l’ACPM final**
...
# ============================================================
# SIDEBAR — Chargement du graphe + choix du nœud de départ
# ============================================================
with st.sidebar:
    ...
    if st.session_state.graph_loaded:
        ...
        if st.session_state.node_list:
            ...
            st.warning("Aucun nœud disponible dans le graphe.")
            st.session_state.start_node = None
...
with col1:
    if st.button("▶️ Démarrer", disabled=st.session_state.running):
        ...
        if G is None:
            ...
            st.error("Choisis un nœud de départ dans la barre latérale.")
        else:
            validate_start_node(G, s)
...
if G is not None:
    ...
    if st.session_state.running and not st.session_state.paused:
        ...
        if st.session_state.step_index < len(st.session_state.steps):
            ...
            fig = plot_prim_step(G, step, st.session_state.is_test_graph)
            graph_placeholder.plotly_chart(fig, use_container_width=True)
    ...
    # ========================================================
        ...
        if st.session_state.finished:
            fig = plot_prim_mst(G, step["mst_edges"], st.session_state.is_test_graph)
        else:
            fig = plot_prim_step(G, step, st.session_state.is_test_graph)
    ...
    # ========================================================
        ...
        graph_placeholder.plotly_chart(fig, use_container_width=True)
        st.info("Choisis un nœud de départ puis clique sur ▶️ Démarrer pour lancer Prim.")
...
L’algorithme de Prim construit un **arbre couvrant minimal (MST)** en :

1. partant d’un nœud initial,  
2. ajoutant à chaque étape l’arête de plus faible poids reliant l’arbre à un nouveau nœud,  
3. jusqu’à couvrir tous les nœuds connectés.
""")
