...
st.title("🌲 Algorithme de Prim")

st.markdown("""
Cette page permet de visualiser l’algorithme de Prim.
...
with st.sidebar:
    ...
    if st.session_state.graph_loaded:
        ...
        # Mode graphe de test → choix du nœud + vitesse
    ...
    with col1:
        if st.button("▶️ Démarrer", disabled=st.session_state.running):
            ...
            if G is None:
                ...
                st.session_state.steps = list(prim_steps(G, s, weight="length"))
                st.rerun()
...
if not st.session_state.is_test_graph:
    ...
    if st.session_state.mst_graph is not None:
        ...
        fig = plot_prim_mst(G, T.edges(data="length"), False)
...
# --- MODE INITIAL ---
    ...
    graph_placeholder.plotly_chart(fig, use_container_width=True)
    st.info("Choisis un nœud de départ puis clique sur ▶️ Démarrer.")
...
L’algorithme de Prim construit un **arbre couvrant minimal (MST)** en ajoutant
à chaque étape l’arête de plus faible poids reliant l’arbre à un nouveau nœud.
""")
